...
int main()
{
	...
	while (!done)
	{
		...
		{
			// Load Level
			cout << "Enter Level you would like to load: ";
			string levelName;
			cin >> levelName;
			...
			else
			{
				...
				RunEditor(pLevel, LevelHeight, LevelWidth);
		...
		else if (input == 2)
		{
			...
			pLevel = new char[LevelHeight * LevelWidth];
...
void RunEditor(char* pLevel, int width, int height)
{
	...
	{
		system("cls");
		DisplayLevel(pLevel, height, width, CursorX, CursorY);
		DisplayLegend();
		DoneEditing = EditLevel(pLevel, CursorX, CursorY, width, height);
	}
	system("cls");
	DisplayLevel(pLevel, height, width, -1, -1);
	DisplayLegend();
...
void DisplayLegend()
{
	...
	cout << "$ for money" << endl;
	cout << "v for moving vertically" << endl;
	cout << "v for moving horiztonally" << endl;
